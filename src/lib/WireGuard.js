...
const WireGuard = class {
  ...
  async __writeCascadeConf(config, h1, h2, h3, h4) {
    ...
    const postUp = `/bin/sh -c '/app/scripts/cascade-in-container-postup.sh ${WG_CASCADE_EXIT_TUNNEL_IP} "${cidrEsc}"'`;
    const preDown = `/bin/sh -c '/app/scripts/cascade-in-container-predown.sh "${cidrEsc}"'`;
