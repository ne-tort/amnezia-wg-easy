...
function writeStreamConfigs(plan) {
  ...
  for (const block of plan.demuxPorts) {
    ...
    const hasMtproto = block.routes.some((r) => r.service === 'mtproto');
    // Telemt behind demux sees nginx's docker IP (172.x) as the client.
    // Fake-TLS/ME need the real client IP → send PROXY protocol to all
    // demux upstreams, and strip it for panel/xray (they do not speak PP).
    const strippers = [];
    let nextStripPort = 13000 + (Number(block.port) % 1000);
    const allocStrip = (upstream) => {
      const listen = nextStripPort;
      nextStripPort += 1;
      strippers.push({ listen, upstream: String(upstream).trim() });
    ...
    for (const r of block.routes) {
      ...
      seen.add(sni);
      const mapped = (hasMtproto && r.service !== 'mtproto') ? allocStrip(up) : up;
      lines.push(`${sni} ${mapped};`);
    }
    lines.push('');
    fs.writeFileSync(mapPath, `${lines.join('\n')}`, 'utf8');

    let defaultUp = block.defaultUpstream || '127.0.0.1:9';
    if (hasMtproto && defaultUp && defaultUp !== '127.0.0.1:9') {
      defaultUp = allocStrip(defaultUp);
    }
    ...
    for (const s of strippers) {
      // Accept PROXY from demux hop, forward raw TCP to the real upstream.
      confParts.push('server {');
      confParts.push(`    listen 127.0.0.1:${s.listen} proxy_protocol;`);
      confParts.push('    resolver 127.0.0.11 valid=10s ipv6=off;');
      confParts.push('    resolver_timeout 5s;');
      confParts.push(`    set $demux_strip_up "${s.upstream}";`);
      confParts.push('    proxy_pass $demux_strip_up;');
      confParts.push('    proxy_connect_timeout 5s;');
      confParts.push('    proxy_timeout 1d;');
      confParts.push('}');
      confParts.push('');
    }
    confParts.push('server {');
    confParts.push(`    listen ${block.port};`);
    confParts.push('    ssl_preread on;');
    confParts.push('    resolver 127.0.0.11 valid=10s ipv6=off;');
    confParts.push('    resolver_timeout 5s;');
    if (hasMtproto) {
      confParts.push('    proxy_protocol on;');
    }
    confParts.push(`    proxy_pass $demux_backend_${block.port};`);
    confParts.push('    proxy_connect_timeout 5s;');
    confParts.push('    proxy_timeout 1d;');
    confParts.push('}');
    confParts.push('');
    fs.writeFileSync(path.join(dir, `demux-${block.port}.conf`), confParts.join('\n'), 'utf8');
  }
