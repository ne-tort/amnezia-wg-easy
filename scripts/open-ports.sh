...
NGINX_MIRROR_HTTPS_PORT="${NGINX_MIRROR_HTTPS_PORT:-}"
...
MIERU_UDP_PUBLIC_PORT="${MIERU_UDP_PUBLIC_PORT:-$MIERU_PUBLIC_PORT}"
ENABLE_MIERU_UDP="${ENABLE_MIERU_UDP:-0}"

PORTS_TCP=("${PANEL_HTTP_PORT}" "${PANEL_HTTPS_PORT}" "${XRAY_PUBLIC_PORT}" "${MIERU_PUBLIC_PORT}")
if [[ -n "$NGINX_MIRROR_HTTPS_PORT" && "$NGINX_MIRROR_HTTPS_PORT" != "$PANEL_HTTPS_PORT" ]]; then
  PORTS_TCP+=("${NGINX_MIRROR_HTTPS_PORT}")
fi
PORTS_UDP=("${WG_PORT}" "${HYSTERIA_PUBLIC_PORT}")
if [[ "$ENABLE_MIERU_UDP" == "1" || "$ENABLE_MIERU_UDP" == "true" || "$ENABLE_MIERU_UDP" == "yes" ]]; then
  PORTS_UDP+=("${MIERU_UDP_PUBLIC_PORT}")
fi
# unique
uniq_tcp=()
