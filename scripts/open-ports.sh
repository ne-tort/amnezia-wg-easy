...
# Reads .env in project root. Defaults: WG_PORT=51820, PANEL_HTTP_PORT=80, PANEL_HTTPS_PORT=10123, XRAY_PORT=443.
...
echo "Opening UDP ${WG_PORT} (VPN), TCP ${PANEL_HTTP_PORT} (HTTP/ACME), TCP ${PANEL_HTTPS_PORT} (HTTPS panel), TCP ${XRAY_PORT} (Xray)..."
...
if command -v ufw >/dev/null 2>&1; then
  ...
  sudo ufw allow "${XRAY_PORT}/tcp" comment 'Amnezia Xray'
  sudo ufw status | grep -E "${WG_PORT}|${PANEL_HTTP_PORT}|${PANEL_HTTPS_PORT}|${XRAY_PORT}" || true
  ...
  sudo firewall-cmd --permanent --add-port="${XRAY_PORT}/tcp"
  ...
  sudo iptables -C INPUT -p tcp --dport "${XRAY_PORT}" -j ACCEPT 2>/dev/null || sudo iptables -I INPUT -p tcp --dport "${XRAY_PORT}" -j ACCEPT
