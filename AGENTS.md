...
`src/lib/portPlan.js` — group by public TCP: shared → stream SNI demux; exclusive → direct `-p`. Named SNI routes: Xray/MT + panel **FQDN** only. Bare IP panel: no named route; demux `default` → panel TLS. FQDN panel: `default` → `:9` (stub/UI on panel SNI).
